...
* `ORMGetSession()` will now throw a `FunctionException` if too many arguments are passed.
* `ORMGetSessionFactory()` will now throw a `FunctionException` if too many arguments are passed.
